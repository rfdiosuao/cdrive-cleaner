        ...
        for protected in self.protected_dirs.iter() {
            if !is_known_cleanup_path && Self::path_is_same_or_child(&path, protected) {
                return PathSafetyResult {
                    ...
                    protection_level: ProtectionLevel::Level1_DirectoryWhitelist,
                };
            }
        }
        ...
        if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
            if self.protected_extensions.contains(&ext.to_lowercase()) {
                return PathSafetyResult {
                    ...
                    protection_level: ProtectionLevel::Level2_ExtensionWhitelist,
                };
            }
        }
            ...
            Ok(info) => {
                if info.is_microsoft {
                    PathSafetyResult {
                        ...
                        protection_level: ProtectionLevel::Level3_SignatureVerification,
                    }
...
pub enum ProtectionLevel {
    None,
    Level1_DirectoryWhitelist,
    Level2_ExtensionWhitelist,
    Level3_SignatureVerification,
    PrivacyProtection,
}
